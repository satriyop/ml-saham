...
| Screen hard filters (replay extract) | `features_by_window.7` only: `candidate.fundamentals.market_cap_idr`, `candidate.fundamentals.piotroski_f_score`, `candidate.accum_score`, `signal.assessment.score` | Counterfactual first-match floors (market_cap → piotroski → accum_score → signal_score); capture may neutralize gates; **not** a production tournament until checkpoint | pass / rejected_flow / rejected_signal / unextractable_contract |
