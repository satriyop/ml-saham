...
| **Screener (pre-open)** | Opening auction / IEV rank | `iev_snapshots` / `iev_snapshot_history`, `learning_observations` (PRE_OPEN…), `candles` | `screener` | **ADR-002:** `screener.pre_open.iev_rank` (IEV table) · `screener.pre_open.directional_score` (PRE_OPEN observations / raw_score). Protocol `pre_open_session_v1`. Legacy: `pre-open-rank`, `pre-open-heuristic`, `pre-open-direction`, `pre-open-participation`, `pre-open-auction`, `pre-open-macro` |
