...
def _pick_history_batches(
    ...
    """Per date keep largest collected_at batch (tie → latest collected_at)."""
    notes: list[str] = []
    ...
    for _date, items in sorted(by_date.items()):
        ...
        if len(by_cap) > 1:
            multi += 1
        best_cap = max(by_cap.keys(), key=lambda c: (len(by_cap[c]), c))
    ...
    if multi:
        notes.append(
            f"iev history: {multi} dates had multiple captures; used largest batch each day"
        )
    return out, notes
    ...
    with connect(path) as conn:
        ...
        for date, items in sorted(by_date.items()):
            ...
            for it in items:
                ...
                iev = _f(it.get("iev"))
                iep = _f(it.get("iep"))
                imb = (iev / iep - 1.0) if iep > 0 else 0.0
                comps = {
